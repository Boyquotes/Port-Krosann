...
# Threshhold for calculation of whether it is a really good buy/sell price
const goodSellTreshhold = 1.8
const goodBuyTreshhold = 0.7
...
class Market:
	...
	func randomizeGoods():
		...
		# Prices
		grainRandPrice = randi()%(grainBasePrice+(int(grainBasePrice*0.1)))+(grainBasePrice-(int(grainBasePrice*0.1)))
		cornRandPrice = randi()%(cornBasePrice+(int(cornBasePrice*0.1)))+(cornBasePrice-(int(cornBasePrice*0.1)))
		vegetablesRandPrice = randi()%(vegetablesBasePrice-(int(vegetablesBasePrice*0.1)))+(vegetablesBasePrice-(int(vegetablesBasePrice*0.1)))
		fruitsRandPrice = randi()%(fruitsBasePrice+(int(fruitsBasePrice*0.1)))+(fruitsBasePrice-(int(fruitsBasePrice*0.1)))
		cottonRandPrice = randi()%(cottonBasePrice+(int(cottonBasePrice*0.1)))+(cottonBasePrice-(int(cottonBasePrice*0.1)))
		beerRandPrice = randi()%(beerBasePrice+(int(beerBasePrice*0.1)))+(beerBasePrice-(int(beerBasePrice*0.1)))
		bricksRandPrice = randi()%(bricksBasePrice+(int(bricksBasePrice*0.1)))+(bricksBasePrice-(int(bricksBasePrice*0.1)))
		cocoaRandPrice = randi()%(cocoaBasePrice+(int(cocoaBasePrice*0.1)))+(cocoaBasePrice-(int(cocoaBasePrice*0.1)))
		coffeeRandPrice = randi()%(coffeeBasePrice+(int(coffeeBasePrice*0.1)))+(coffeeBasePrice-(int(coffeeBasePrice*0.1)))
		sugarRandPrice = randi()%(sugarBasePrice+(int(sugarBasePrice*0.1)))+(sugarBasePrice-(int(sugarBasePrice*0.1)))
		tabaccoRandPrice = randi()%(tabaccoBasePrice+(int(tabaccoBasePrice*0.1)))+(tabaccoBasePrice-(int(tabaccoBasePrice*0.1)))
		silkRandPrice = randi()%(silkBasePrice+(int(silkBasePrice*0.1)))+(silkBasePrice-(int(silkBasePrice*0.1)))
